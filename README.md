...
The runnable entry point creates the boolean flag `creator-content-processing-v2`, reads it, counts delivery stages, and captures a processing exception with creator and asset context. The write requests carry a client-generated `idempotency_key`; a retried report or capture keeps one operation identity. Helps when a rate limiter eats your retry and you need to trace it.
...
There is no SDK dependency here. The client is a short Python HTTP helper, and its call sites read as `infrai.flags.set`, `infrai.metrics.report`, and `infrai.errors.capture`. You can call it from any language with a plain REST request, no vendor lock-in.
